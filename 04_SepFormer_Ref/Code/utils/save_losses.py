...
def load_npy_test_data(data_set):
    path_to_test_data_sets = os.path.join('/isip/Public/spruenken', data_set, 'wav8k/min/test')
...
if __name__ == '__main__':
    ...
    # Parameters
    model = TinySepformer()
    all_models = '/isip/Users/spruenken/Documents/Models' # '/isip/Users/spruenken/Documents/Models' -- '/isip/Public/spruenken/Documents/Models/'
    model_weights = '2023-07-14_22-25-09'
    data_set = 'LibriMix'  # 'LibriMix' -- 'wsj0-2mix'
    intermediate_path = True
    epoch = 48  # only necessary if intermediate status is to be tested
    batch_size = 8
    ...
    # Load the model
    output = model(tf.ones([1, 8000, 1]))
    model.load_weights(entire_path)

    # Test the model and save all losses
    loss_history = LossHistory()
    model.evaluate(test_dataset, callbacks=[loss_history])

    # Print all loss-values and shape
    print('Shape', len(loss_history.losses))
    print('All losses', loss_history.losses)
    # Save losses as .npy
    save_path = '/isip/Users/spruenken/Documents/Losses/ts_288_librimix_librimix.npy'
    loss_ndarray = np.array(loss_history.losses)
    np.save(save_path, loss_ndarray)
